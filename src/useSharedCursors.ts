...
import {useSharedCursorsType, CursorDataType, CursorCoordinates, CursorHookType, } from './types';
import { useCookieBackedUserId } from './useCookieBackedUserId';
import { useCursorEvents } from './useCursorEvents';

export function useSharedCursors<T = void>(args: useSharedCursorsType<T>): CursorHookType<T> {
	const [cursors, setCursors] = useState<Array<CursorDataType<T>>>([]);
	const [myCursor, setMyCursor] = useState<CursorCoordinates | undefined>(undefined);
